package com.pic.catcher.ui.wrapper
...
import com.pic.catcher.util.ext.contains
import java.net.URL
...
class WebViewComponent(context: Context) {
    ...
    private var webviewComponentCallBack: com.pic.catcher.ui.wrapper.WebViewComponentCallBack? = null
    ...
    fun loadUrl(url: String, callBack: com.pic.catcher.ui.wrapper.WebViewComponentCallBack? = null) {
    ...
    class WebUrlInterceptor() {
        fun shouldOverrideUrlLoading(view: WebView?, uri: Uri?): Boolean {
            ...
            if (uri.toString().matches(Regex("https?://.+\\.(?:apk|zip|rar|gzip)", RegexOption.IGNORE_CASE))) {
                AppRouter.route(view.context, uri.toString())
                return true
            }
